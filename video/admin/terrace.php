...
$type=$_POST[type];
if($fenlei==0)
{$type="(`id`, `name`, `type`, `big_id`, `small_id`, `sort`) VALUES (null, '$name', '$fenlei', '0', '0', '$sort')";}
else
{$type="(`id`, `name`, `type`, `big_id`, `small_id`, `link`, `sort`) VALUES (null, '$name', '$type', '0', '$fenlei', '$link', '$sort')";}
dbinsert(uboterrace,$type);
echo msglayerurl("添加成功，返回页面",5,"terrace.php");
}
$id=$_GET[id];
$type="WHERE id='$id'";
$duqu=queryall(uboterrace,$type);
if($_POST[edit]){
$name=$_POST[name];
$fenlei=$_POST[fenlei];
$sort=$_POST[sort];
...
$type=$_POST[type];
if($fenlei==0)
{$type="name='$name',sort='$sort'  where id='$id'";}
else
{$type="name='$name',type='$type',small_id='$fenlei',link='$link',sort='$sort'  where id='$id'";}
upalldt(uboterrace,$type);
