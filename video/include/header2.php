...
<div class="left_nav">
<a class="fl" href="/" _hover-ignore="1">首页<i></i></a><span class="sep">|</span>
<?php
$query = mysql_query("SELECT * FROM se2fl   limit 6 ");
while($a = mysql_fetch_array($query)) {?>
<a href='/list.php?flid=<?php echo $a[id]?>' _hover-ignore="1"><?php echo $a[name]?><i></i></a><span class="sep">|</span>
<?php }?>
</div><ul class="fr"><li class="sign_panel"><div class="left_nav" style="float:right"><a href="/idea.php" class="signin sign_btn signin normal-a dosign"><em></em>意见反馈<i></i></a></div></li></ul><div class="sep" style="float:right">|</div><div class="left_nav" style="float:right"><a href="/tuku/" class="fl on">美女图库</a></div></div></div></div>

<div class="jiu-nav-main" _hover-ignore="1"><div class="jiu-nav w980"><div class="nav-item fl"><div class="item-list">
<ul>
<li><a href="/tuku/" <?php if(empty($pdid)){ ?>class="active"<?php }  ?>>全部</a></li>
<?php
$query = mysql_query("SELECT * FROM se2tufl order by id limit 6 ");
while($a = mysql_fetch_array($query)) {?>
<li><a href="/tuku/list.php?tid=<?php echo $a[id]?>" title="<?php echo $a[name]?>" <?php if($pdid==$a[id] || $fenlei==$a[id]){ ?>class="active"<?php }else{  ?><?php }  ?>><?php echo $a[name]?></a></li>
<?php }
...
$xmttj=$row2[0];
?>   
</ul></div></div><div class="nav-other fl"><ul><li><a href="/list.php?m=new"><i class="today"></i><span>今日视频</span></a><em><?php echo $xsptj;?></em></li><li><a href="/tuku/list.php?m=new"><i class="tw"></i><span>今日美图</span></a><em><?php echo $xmttj;?></em></li></ul></div><div class="n_h"><span>排序：</span><a href="list.php<?php if ($pdid){echo '?tid='.$pdid;} if($m){echo '?m=new';}?>" <?php if (empty($sort) || $sort=='default'){echo 'class="active"';}?>>默认</a><a href="list.php?<?php if ($pdid){echo 'tid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=new" <?php if ($sort=='new'){echo 'class="active"';}?>>最新</a><a href="list.php?<?php if ($pdid){echo 'tid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=heat" <?php if ($sort=='heat'){echo 'class="active"';}?>>人气</a><a href="list.php?<?php if ($pdid){echo 'tid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=price" <?php if ($sort=='price'){echo 'class="active"';}?>>金币</a><a href="list.php?<?php if ($pdid){echo 'tid='.$pdid.'&';} if($m){echo 'm=new&';}?>sort=hot" <?php if ($sort=='hot'){echo 'class="active"';}?>>点赞</a></div></div></div>
