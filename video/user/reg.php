...
$user=getone("select * from ubouser WHERE user='".$username."' or email='".$email."'");
if(empty($user))
{
$time=time();
...
$type="(`id`, `user`, `pass`, `email`, `userid`, `avatar`, `ip`, `zctime`, `hylx`, `kstime`, `endtime`) VALUES (null,'$username','$newpass','$_POST[email]','$userid','$avatar','$ip','$time','$hylx','$time','$endtime')"; 
}
else
{
$type="(`id`, `user`, `pass`, `email`, `userid`, `avatar`, `ip`, `zctime`) VALUES (null,'$username','$newpass','$_POST[email]','$userid','$avatar','$ip','$time')"; 
}
...
<script language="javascript">
	...
	function CheckUser()
	{
		if(!checkLength("username" , "用户名称" , 3 , 16 , "~!@#$%^&*+=\\\'\"\<\>"))
			return false;
		window.msgubotj.location.href = 'jc.php?username=' + RegForm.username.value;
	}
		function Checkemail()
	{
...
<INPUT id=btn_Email onClick="Checkemail();" type=button value=检测邮箱 class="user_reg_but" >
</div>
