...
$i=1;
?>
{"status":1,"msg":"","data":[<?php while ($a=mysql_fetch_array($query)) { ?>{"k":"<?php echo $i;?>","id":"<?php echo $a[id];?>","title":<?php $name=$a[name];$name=str_replace("?","",$name); echo json_encode(iconv('gb2312','utf-8',$name))?>,"pic_url":"<?php echo $a[pic]?>","cishu":"<?php echo $a[cishu]?>","member":"<?php echo $a[member]?>","source":"<?php echo $a[source]?>","addtime":"<?php $addtime=date('m-d',$a[addtime]); echo $addtime;?>","hits":"<?php echo $a[hits]?>"}<?php if ($i !== $dqjl){?>,<?php }?><?php 
$i++;  
};?>],"dialog":""}