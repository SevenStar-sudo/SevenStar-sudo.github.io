...
$i=1;
?>
{"status":1,"msg":"","data":[<?php while ($a=mysql_fetch_array($query)) { $source=$a[source]; switch ($source){ case '0': $pyname= '优酷'; break; case '1': $pyname= '土豆'; break; case '2': $pyname= '腾讯'; break; case '3': $pyname= '乐视'; break;case '4': $pyname= '芒果'; break;case '5': $pyname= '爱奇艺';break; case '6' : $pyname= '其他'; break;} ?>{"k":"<?php echo $i;?>","id":"<?php echo $a[id];?>","title":<?php echo json_encode(iconv('gb2312','utf-8',$a[name]))?>,"status":<?php echo json_encode(iconv('gb2312','utf-8',$a[shijian]))?>,"member":"<?php echo $a[member]?>","source":<?php echo json_encode(iconv('gb2312','utf-8',$pyname))?>,"pic_url":"<?php echo $a[pic]?>","cishu":"<?php echo $a[cishu]?>","hits":"<?php echo $a[hits]?>"}<?php if ($i !== $dqjl){?>,<?php }?><?php 
$i++;  
};?>],"dialog":""}