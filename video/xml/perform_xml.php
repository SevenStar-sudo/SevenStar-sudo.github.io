...
$i=1;
?>
{"status":1,"msg":"","data":[<?php while ($a=mysql_fetch_array($query)) { ?>{"k":"<?php echo $i;?>","id":"<?php echo $a[id];?>","title":<?php $name=$a[name];$name=str_replace("?","",$name); echo json_encode(iconv('gb2312','utf-8',$name))?>,"pic_url":"<?php echo $a[pic]?>","cishu":"<?php echo $a[cishu]?>","member":"<?php echo $a[member]?>","source":"<?php echo $a[source];?>","diqu":<?php $diqu=$a[diqu]; echo json_encode(iconv('gb2312','utf-8',$diqu))?>,"room":"<?php $room=$a[room]; echo $room;?>","enroll":"<?php $enroll=$a[enroll]; echo $enroll;?>","state":"<?php $xcstate=$a[xcstate];echo $xcstate;?>","reveal":"<?php if ($xcstate==1){if ($state==0){ echo "\u5df2\u7ed3\u675f"; }else{ echo "\u76f4\u64ad\u4e2d"; }}else{ $ptime=$a['addtime'];$ptime=date('H:i',$ptime);$tdate=date("Y-m-d")." ".$ptime.":01";$dtae=intval(strtotime($tdate));$nexttime=date("Y-m-d")." ".date('H:i:s',strtotime("60 minute",$dtae));$time=time();$dtae1=strtotime($tdate);$dtae2=strtotime($nexttime); if ($xcstate==2){if ($state==0){ echo "\u5df2\u7ed3\u675f"; }else{if($time>$dtae1 && $dtae2>$time){echo "\u76f4\u64ad\u4e2d";}else{if ($dtae2<$time){echo "\u5df2\u7ed3\u675f";}else{echo  "<i></i>".date('H:i',$a[addtime]);}}}}else{echo "\u4eba\u6ee1\u5f00\u59cb";}}?>","price":"<?php $price=$a[price]; echo $price;?>","addtime":"<?php $addtime=date('m-d',$a[addtime]); echo $addtime;?>","hits":"<?php echo $a[hits]?>"}<?php if ($i !== $dqjl){?>,<?php }?><?php 
$i++;  
};?>],"dialog":""}